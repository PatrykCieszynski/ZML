...
Screen capture and OCR originally ran inside Game Bridge. The OCR code owns
...
package shared by Game Bridge and OCR Agent through local path
dependencies. Game Bridge depends only on this contract package; the standalone
OCR Agent is a runtime process dependency rather than a Python dependency.
...
logic, durable domain events, and persistence. OCR Agent owns applied
...
The OCR runner emits protocol observations. A Game Bridge input adapter maps
...
- The OCR pipelines live in an independently verified `apps/ocr-agent` project.
- Game Bridge never imports `zml_ocr_agent` and does not install its native OCR
...
- A missing or crashing Agent degrades OCR health and triggers bounded restart
  without terminating Game Bridge.
