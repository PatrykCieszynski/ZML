...
export type BackendClient = {
  getHealth: () => Promise<BackendHealthDto>;
...
export class BackendRestClient implements BackendClient {
  private readonly baseUrl: URL;
  private readonly timeoutMs: number;
  private readonly fetchImpl: FetchLike;

  constructor({ baseUrl, timeoutMs = 3_000, fetchImpl = fetch }: BackendRestClientOptions) {
    this.baseUrl = new URL(normalizeBaseUrl(baseUrl));
    this.timeoutMs = timeoutMs;
    this.fetchImpl = fetchImpl;
  }

  async getHealth(): Promise<BackendHealthDto> {
    const data = await this.getJson("/health");
    if (!isBackendHealthWire(data)) {
      throw new Error("Agent /health returned an invalid payload");
    }
    return wireToBackendHealthDto(data);
  }
