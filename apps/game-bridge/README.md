...
- `ZML_OCR_AGENT_PATH`: optional path to the standalone OCR Agent executable;
  direct backend runs resolve `zml-ocr-agent` from `PATH` when this is unset.
...
artifacts. Electron supplies `ZML_OCR_AGENT_PATH` for both development and
packaged runs. Game Bridge never imports the Agent package; the executable is a
