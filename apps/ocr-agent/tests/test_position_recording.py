...
def test_position_roi_snapshot_recorder_overwrites_fixed_files(tmp_path: Path) -> None:
    recorder = PositionRoiSnapshotRecorder(
        config=PositionRoiSnapshotConfig(
            enabled=True,
            root_dir=tmp_path,
            interval_ms=60_000,
            max_samples=2,
    ...
    assert int(_read_image(tmp_path / "compass.png").mean()) == 10
    ...
    assert int(_read_image(tmp_path / "compass.png").mean()) == 200
    assert _read_image(tmp_path / "lon.png").shape[:2] == (2, 3)
    assert _read_image(tmp_path / "lat.png").shape[:2] == (3, 3)
...
def test_position_roi_snapshot_recorder_respects_max_samples(tmp_path: Path) -> None:
    recorder = PositionRoiSnapshotRecorder(
        config=PositionRoiSnapshotConfig(
            enabled=True,
            root_dir=tmp_path,
...
def test_position_roi_snapshot_config_from_env(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setenv("ZML_POSITION_ROI_SNAPSHOT_MAX_SAMPLES", "3")
