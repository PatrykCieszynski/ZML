...
def test_version_command() -> None:
    result = subprocess.run(
        [sys.executable, "-m", "zml_ocr_worker", "--version"],
        check=True,
        capture_output=True,
        text=True,
    )

    assert result.stdout.strip() == "zml-ocr-worker 0.1.0"
...
def test_stdio_process_emits_hello_accepts_shutdown_and_exits() -> None:
    ...
    process = subprocess.Popen(
        [sys.executable, "-m", "zml_ocr_worker", "stdio"],
...
def test_agent_source_does_not_import_game_bridge() -> None:
    source_root = Path(__file__).parents[1] / "src" / "zml_ocr_worker"
    offenders = [
        path
        for path in source_root.rglob("*.py")
        if "zml_backend" in path.read_text(encoding="utf-8")
    ]
