...
def test_protocol_session_requires_hello_before_observations() -> None:
    ...
    with pytest.raises(OcrWorkerProcessError, match="first OCR Worker message must be hello"):
...
def test_protocol_session_rejects_fatal_agent_status_and_non_monotonic_sequence() -> None:
    ...
    with pytest.raises(OcrWorkerProcessError, match="tesserocr unavailable"):
        session.handle(
            StatusMessage(
                protocol_version=1,
                type="status",
    ...
    with pytest.raises(OcrWorkerProcessError, match="increase monotonically"):
...
def test_supervisor_restarts_agent_and_accepts_position_and_finder_after_restart(
    ...
    source = OcrWorkerSupervisor(
        config=OcrWorkerSupervisorConfig(
            enabled=True,
            process=OcrWorkerProcessConfig(command=("fake-agent",), environment={}),
