...
from zml_backend.dev_cli import InputMode, _apply_env_overrides, app
...
def test_apply_env_overrides_sets_mock_mode(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.delenv("ZML_FINDER_PRESENCE_CHECK", raising=False)
    ...
    monkeypatch.delenv("ZML_POSITION_ROI_SNAPSHOT_MAX_SAMPLES", raising=False)
    ...
    assert os.environ["ZML_FINDER_RECORDING"] == "interval"
    ...
    assert os.environ["ZML_FINDER_RECORDING_MAX_SAMPLES"] == "3"
    assert os.environ["ZML_FINDER_PRESENCE_CHECK"] == "0"
    ...
    assert os.environ["ZML_POSITION_ROI_SNAPSHOT_MAX_SAMPLES"] == "2"
...
def test_config_command_prints_resolved_settings(monkeypatch, tmp_path: Path) -> None:
    ...
    assert "Z Mining Log Backend config" in result.output
    ...
    assert "finder_presence_check_enabled" in result.output
    assert "position_roi_snapshot_enabled" in result.output
    assert "position_roi_snapshot_max_samples" in result.output
