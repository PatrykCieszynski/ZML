...
class OcrWorkerProcessError(RuntimeError):
...
@dataclass(frozen=True, slots=True)
class OcrWorkerSupervisorConfig:
    enabled: bool
...
class OcrWorkerSupervisor:
    """Run OCR Worker as a restartable child while implementing OcrInputSource."""
    ...
    @property
    def config(self) -> OcrWorkerSupervisorConfig:
        return self._config
    ...
    def _run(self, *, stop_event: threading.Event) -> None:
        ...
        while not stop_event.is_set():
            ...
            self._supervisor.mark_degraded(
                _OCR_WORKER_NAME,
                f"OCR Worker process failed; restart {restart_count} in {delay_s:.2f}s: {error_text}",
            )
            logger.warning(
                "ocr_worker_restart_scheduled restart_count=%s delay_s=%.2f error=%s",
                restart_count,
                delay_s,
                error_text,
            )
        ...
        *,
        ...
        try:
            ...
            return _ProcessRunResult(
                error=OcrWorkerProcessError(f"failed to start child: {exc}"),
                uptime_s=0.0,
            )
        ...
        self._supervisor.mark_degraded(_OCR_WORKER_NAME, "OCR Worker handshake pending")
        ...
        *,
        ...
        while not stop_event.is_set():
            ...
            if exit_code is not None:
                return OcrWorkerProcessError(f"child exited before hello with code {exit_code}")
            if session.handshake_complete.is_set():
                return None
            if self._monotonic() >= deadline:
                return OcrWorkerProcessError("hello handshake timed out")
    ...
    def _apply_desired_config(
        ...
        *,
        ...
        try:
            ...
            return OcrWorkerProcessError(f"failed to send apply_config: {exc}")
    ...
    def _monitor_process(
        ...
        *,
        ...
        while not stop_event.is_set():
            ...
            exit_code = transport.poll()
            if exit_code is not None:
                return OcrWorkerProcessError(f"child exited unexpectedly with code {exit_code}")
            heartbeat_age_s = session.heartbeat_age_s()
            if heartbeat_age_s > self._config.heartbeat_timeout_s:
                return OcrWorkerProcessError(f"heartbeat timed out after {heartbeat_age_s:.2f}s")
            stop_event.wait(self._config.monitor_interval_s)
        return None
...
class _ProtocolSession:
    ...
    def expect_config(self, *, command_id: str, revision: int) -> None:
        with self._lock:
            if not self._hello_received:
                raise OcrWorkerProcessError("cannot configure OCR Worker before hello")
            if self._expected_config_command_id is not None:
                raise OcrWorkerProcessError("OCR Worker config command is already pending")
        ...
        with self._lock:
            if not self._hello_received:
                if not isinstance(message, HelloMessage):
                    raise OcrWorkerProcessError("first OCR Worker message must be hello")
                missing = _REQUIRED_CAPABILITIES.difference(message.payload.capabilities)
                if missing:
                    missing_text = ", ".join(sorted(missing))
                    raise OcrWorkerProcessError(
                        f"OCR Worker is missing required capabilities: {missing_text}"
                    )
            ...
            if isinstance(message, HelloMessage):
                raise OcrWorkerProcessError("OCR Worker emitted duplicate hello")
            if message.sequence_id <= self._last_sequence_id:
                raise OcrWorkerProcessError(
                    "OCR Worker sequence_id must increase monotonically "
    ...
    def _handle_command_result(self, message: CommandResultMessage) -> None:
        ...
        if command_id is None or revision is None:
            raise OcrWorkerProcessError("OCR Worker emitted an unexpected command_result")
        payload = message.payload
        if payload.command_id != command_id or payload.command_type != "apply_config":
            raise OcrWorkerProcessError("OCR Worker returned command_result for another command")
        if payload.status == "error":
            error = payload.error
            detail = "unknown error" if error is None else f"{error.code}: {error.message}"
            self.config_error = OcrWorkerProcessError(
                f"OCR Worker rejected config revision {revision}: {detail}"
            )
            self.config_complete.set()
            return
        if payload.applied_revision != revision:
            raise OcrWorkerProcessError(
                "OCR Worker acknowledged unexpected config revision "
    ...
    def _require_configured_observations(self) -> None:
        if not self.config_complete.is_set() or self.config_error is not None:
            raise OcrWorkerProcessError(
                "OCR Worker emitted an observation before apply_config completed"
            )
    ...
    def _apply_agent_state(
        ...
        *,
        ...
        if code in _FATAL_AGENT_STATUS_CODES:
            message = detail or f"OCR Worker reported fatal status {code}"
            self._supervisor.update_details(
                _OCR_WORKER_NAME,
            ...
            raise OcrWorkerProcessError(message)
        if state == "running":
            self._supervisor.update_details(
                _OCR_WORKER_NAME,
        ...
        message = detail or "OCR Worker reported degraded state"
        self._supervisor.update_details(
            _OCR_WORKER_NAME,
    ...
    *,
    ...
    try:
        while True:
            ...
            if not line:
                failures.put(OcrWorkerProcessError("OCR Worker stdout closed"))
                return
            try:
                message = decode_agent_message(line)
                session.handle(message)
            except (OcrProtocolError, OcrWorkerProcessError, ValueError) as exc:
                failures.put(exc)
                return
    except Exception as exc:
        failures.put(OcrWorkerProcessError(f"stdout reader failed: {exc}"))
    ...
    try:
        while True:
            ...
            if not line:
                return
            logger.info(
                "ocr_worker_stderr line=%s",
                line.decode("utf-8", errors="replace").rstrip(),
            )
    except Exception:
        logger.warning("ocr_worker_stderr_reader_failed", exc_info=True)
...
def _stop_process_gracefully(transport: OcrProcessTransport) -> None:
    try:
        ...
        logger.debug("ocr_worker_shutdown_command_failed", exc_info=True)
    try:
        transport.close_stdin()
    except Exception:
        logger.debug("ocr_worker_shutdown_stdin_close_failed", exc_info=True)
...
def _force_stop_process(transport: OcrProcessTransport) -> None:
    try:
        ...
        logger.debug("ocr_worker_close_stdin_failed", exc_info=True)
    if transport.wait(timeout_s=0.5) is not None:
        return
    try:
        transport.terminate()
    except Exception:
        logger.debug("ocr_worker_terminate_failed", exc_info=True)
    if transport.wait(timeout_s=1.0) is not None:
        return
    try:
        transport.kill()
    except Exception:
        logger.warning("ocr_worker_kill_failed", exc_info=True)
